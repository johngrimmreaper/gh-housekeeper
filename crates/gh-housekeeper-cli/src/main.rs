    ...
    let interval = command
        ...
        .with_context(|| {
            format!(
                ...
                command.interval.as_deref().unwrap_or_default()
            )
        })?;
    let output = match command.format {
        OutputFormat::Table => DaemonOutputFormat::Table,
        OutputFormat::Json => DaemonOutputFormat::Json,
    };

    run_foreground_daemon(
        provider,
        ForegroundDaemonOptions {
            scan_options: command.scope.scan_options(),
            monitoring_interval_override: interval,
            max_attempts: (command.iterations > 0).then_some(command.iterations),
            output,
            presentation: DaemonPresentation::MonitorWatch,
        },
    )
    .await
}
