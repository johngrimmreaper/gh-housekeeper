...
The headless runtime is documented in `DAEMON.md`. The dedicated foreground `gh-housekeeperd` now owns scheduling/lifecycle through the same shared runtime used by `monitor watch`. On Unix, a local request/reply control adapter exposes live status and graceful shutdown through the runtime's `DaemonControlHandle`; the socket is not a second scheduler, lock, or business-logic implementation. CLI, GUI, daemon, future system service, and automation must continue to call the same core policy/planning/revalidation/execution services.
...
The receipt API is deliberately named `record_delivery_if_new`: a receipt records a notification that has already been successfully delivered. It is neither authorization to send nor a cleanup authorization.

`AccountUsagePollingService` is the shared application-layer orchestrator for automatic polling. A cycle receives its billing period and evaluation time explicitly, groups configured exact-SKU allowances by billing owner, performs one provider request per owner/period, persists the resulting observation, then evaluates every allowance for that owner against the same observation. Provider `unknown`/`unsupported`, stale, healthy, ambiguous, and unconfigured results never become quota-notification candidates; only warning and critical results do.

Notification delivery is adapter-based. The service looks up the exact `UsageQuotaAlertKey` before delivery, suppresses already-delivered keys, fails closed when receipt history is indeterminate, calls the delivery adapter only for a known-undelivered candidate, and records the receipt only after the adapter reports success. A delivery failure therefore leaves no receipt and can be retried by a later cycle.

The current foreground integration lives in `monitor watch`. It keeps artifact monitoring on the existing `MonitoringScheduler` and runs account-usage polling at the independently configured `account_usage.check_interval_minutes` under the same cancellation lifecycle. The runtime computes and passes the current UTC monthly `BillingPeriod` at the orchestration boundary rather than hiding wall-clock lookup inside quota-domain logic. The current notification adapter writes a foreground diagnostic/notification to stderr; it is not a desktop notification implementation.

Long-running foreground monitoring acquires a stable process lock under the platform state directory before starting. This prevents two cooperating gh-housekeeper long-running instances from both observing `NotDelivered` and racing through delivery before either writes its receipt. The dedicated `gh-housekeeperd` executable now reuses that exact lock and runtime. Its Unix control socket is created only after the lock is held and is never treated as singleton authority. Service-manager integration and the GUI launch path remain future presentation/runtime work and must reuse the same single-instance policy rather than introduce another scheduler or lock family.
