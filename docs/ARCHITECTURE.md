...
Long-running foreground monitoring acquires a stable process lock under the platform state directory before starting. This prevents two cooperating gh-housekeeper long-running instances from both observing `NotDelivered` and racing through delivery before either writes its receipt. The dedicated `gh-housekeeperd` executable, service-manager integration, and GUI launch path are still future presentation/runtime work and must reuse the same single-instance policy rather than introduce another scheduler or lock family.
