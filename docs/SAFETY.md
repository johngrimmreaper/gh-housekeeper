...
Run policy can count elapsed `keep_days` or UTC Monday–Friday `keep_business_days`, never both in the same defaults block or rule. A rule declaring either mode replaces the run default mode; equally specific conflicting retention requests require manual review. `keep_latest_by = "workflow"` narrows the newest-N safety allowance to all matching branches within each repository/workflow ID, while the legacy grouping remains the default. Exact local protection still takes precedence. These settings affect only classification and exact target selection: the existing plan schema, remote revalidation, write-ahead intent, authorization, audit, and post-delete verification remain in force. A plan created before these policy options is still assessed against current local protections at revalidation and apply.
...
The daemon protocol is deliberately non-destructive. It exposes status, run-monitoring-now, graceful shutdown, and notification/status events. The current Unix request/reply transport makes only status and graceful shutdown operational for CLI clients; run-monitoring-now returns structured `unsupported` until the existing scheduler gains a safe wake mechanism. Automatic cleanup remains disabled, and the capability set does not advertise destructive cleanup.

Future daemon automation must call the same shared policy, immutable planning, revalidation, authorization, write-ahead intent, mutation, verification, and audit services used by direct CLI operation. The current Unix socket, future D-Bus/named-pipe transports, tray, GUI, and service-manager integrations are transport and presentation layers only. The Unix endpoint uses a private directory and socket permissions, refuses symlink/non-socket collisions, bounds request frames, and will not replace an endpoint that is accepting connections. A transport request must never contain or infer an unchecked replacement deletion target.
...
The explicit one-shot CLI never records a delivery receipt merely because an evaluation is warning or critical. Automatic delivery is confined to the long-running foreground monitoring path: `monitor watch` invokes the shared account-usage poller only when account-usage allowances are explicitly configured. The poller remains GET-only, persists the provider observation first, and never hands quota state to a cleanup planner or executor.

For automatic delivery, receipt lookup precedes the notification adapter. An existing receipt suppresses delivery. Indeterminate receipt history fails closed and sends nothing. `record_delivery_if_new` runs only after the adapter reports successful delivery; adapter failure writes no receipt so a later cycle can retry. The current adapter is only a foreground stderr notification/diagnostic sink, not a desktop-notification service.

The receipt lookup/delivery/receipt sequence is deliberately protected by a long-running process lock because it is not transactional across processes. The current foreground runtime holds `daemon/instance.lock` in the platform state directory for its lifetime and refuses a second cooperating instance immediately. `gh-housekeeperd` already uses that same singleton policy; its Unix socket is only a control endpoint and does not replace the lock. Future GUI-started or service-manager launch paths must acquire the same lock before automatic quota delivery; they must not mark an alert delivered before the adapter actually succeeds.
